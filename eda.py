...
# ── chart 1: "How bad is our churn problem?" ─────────────────────────────────

def plot_churn_crisis(df: pd.DataFrame) -> None:
    """
    Business question: How critical is churn to our revenue?
    Shows the churn rate AND translates it into annual revenue lost.
    """
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle('How Critical Is Our Churn Problem?',
    ...
    # Right: annual revenue at risk by plan
    plan_avg_fee = df.groupby('plan_type')['monthly_fee'].first()  # fee is fixed per plan
    ...
    axes[1].set_title(f'Annual Revenue Lost to Churn: ${total/1e6:.1f}M',
                      fontsize=11, color=RED, fontweight='bold')
...
# ── chart 2: "Which customers are we losing?" ────────────────────────────────

def plot_who_is_leaving(df: pd.DataFrame) -> None:
    """
    Business question: Which customer segments have the highest churn?
    Helps prioritise which segment to focus retention efforts on.
    """
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle('Which Customers Are We Losing?',
                 fontsize=16, fontweight='bold', y=1.01)
    ...
    bars = axes[0].bar(plan_churn.index, plan_churn.values,
                       color=[RED if v > overall else BLUE for v in plan_churn.values],
    ...
    df2 = df.copy()
    df2['tenure_bin'] = pd.cut(df2['tenure_months'], bins=[0, 6, 12, 24, 200],
    ...
    bars = axes[1].bar(tenure_churn.index, tenure_churn.values,
                       color=[RED if v > overall else BLUE for v in tenure_churn.values],
                       edgecolor='white', width=0.5)
    ...
    # By usage
    usage_labels = ['<5h\n(Inactive)', '5–10h\n(Low)', '10–15h\n(Medium)', '15h+\n(High)']
    df2['usage_bin'] = pd.cut(df2['avg_weekly_usage_hours'], bins=[0, 5, 10, 15, 100],
                              labels=usage_labels, include_lowest=True)
    ...
    bars = axes[2].bar(usage_churn.index, usage_churn.values,
                       color=[RED if v > overall else BLUE for v in usage_churn.values],
                       edgecolor='white', width=0.5)
...
# ── chart 3: "What are the warning signs?" ───────────────────────────────────

def plot_warning_signs(df: pd.DataFrame) -> None:
    """
    Business question: What behaviours separate churners from loyal customers?
    These become the intervention triggers for the retention team.
    """
    signals = {
    ...
    fig.suptitle('Early Warning Signs: What Separates Churners from Loyal Customers?',
...
def plot_correlation_heatmap(df: pd.DataFrame) -> None:
    """
    Business question: Which metrics are most correlated with churn?
    Validates that we're tracking the right KPIs.
    """
    df_num = df.copy()
...
def print_business_summary(df: pd.DataFrame) -> None:
    ...
    for label, col, direction in [
        ...
        diff = abs(c - r)
        print(f"  {label:<25}  churned={c:.1f}   retained={r:.1f}   gap={direction}{diff:.1f}")
    print("═" * 60)
