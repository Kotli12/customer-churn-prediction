...
# ── step 1: load & frame the business problem ─────────────────────────────────
...
def load_data():
    ...
    rev_lost   = n_churned * avg_fee * 12

    _section("STEP 1 — BUSINESS PROBLEM")
    print(f"  Customers in dataset : {n_total:,}")
    ...
    print(f"  Revenue at risk      : ${rev_lost/1e6:.1f}M per year")
    print()
...
# ── step 3: why these models ──────────────────────────────────────────────────

def explain_models() -> None:
    _section("STEP 3 — WHY THESE MODELS?")
    rows = [
        ("Logistic Regression",
         "Baseline. Simple, fast, interpretable. Sets the floor."),
        ("Random Forest",
         "Ensemble of decision trees. Gives feature importance — tells us *which* "
         "factors drive churn, not just that it happens."),
        ("Gradient Boosting",
         "Builds trees sequentially, each one fixing the last one's mistakes. "
         "Strong on tabular data."),
        ("XGBoost",
         "Optimised gradient boosting. Handles class imbalance via scale_pos_weight."),
        ("LightGBM",
         "Fastest of the ensemble methods. Good when we later retrain on more data."),
    ]
...
# ── step 4: why recall matters more than precision ────────────────────────────

def explain_metric_choice(avg_annual_value: float) -> None:
    _section("STEP 4 — WHY WE OPTIMISE FOR RECALL, NOT ACCURACY")
    cost_false_neg  = avg_annual_value          # missing a churner = lose their full value
    cost_false_pos  = 50                         # false alarm = cost of one outreach call
    print(f"  Missing a churner (False Negative) costs : ${cost_false_neg:,.0f}/year")
    print(f"  False alarm (False Positive) costs       : ${cost_false_pos}/outreach call")
    print()
...
def train_all_models(X, y, df):
    ...
    pos_weight = (y == 0).sum() / (y == 1).sum()  # for XGBoost imbalance correction
    ...
    for name, model in models.items():
        cv_auc = cross_val_score(model, X_tr_s, y_train, cv=cv,
        ...
        r = results[name]
        print(f"  {name:<22} {r['accuracy']:>9.3f} {r['recall']:>8.3f} "
              f"{r['roc_auc']:>9.3f} {cv_auc:>8.3f}")

    return results, X_test, y_test, scaler


# ── step 6: pick best model & explain the choice ──────────────────────────────

def select_best(results: dict, y_test, df) -> tuple:
    ...
    print(f"  ROC-AUC     : {best['roc_auc']:.3f}  "
          f"(ranks a real churner above a non-churner {best['roc_auc']*100:.0f}% of the time)")
    print(f"  Recall      : {best['recall']:.3f}  "
          f"(we catch {best['recall']*100:.0f}% of customers who would have churned)")
    print(f"  Precision   : {best['precision']:.3f}  "
          f"(of those we flag, {best['precision']*100:.0f}% are genuine churners)")
    print()

    # Business impact math
    n_churned    = (y_test == 1).sum() * 5   # scale up from 20% test set
    caught       = int(n_churned * best['recall'])
    save_rate    = 0.40                       # assume 40% of caught churners convert
    saved        = int(caught * save_rate)
    revenue_saved= saved * avg_fee * 12
    campaign_cost= caught * 50               # $50 outreach per flagged customer
...
def plot_results(results: dict, y_test, best_name: str) -> None:
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Model Evaluation — Customer Churn Prediction',
    ...
    axes[0, 0].set_title('Model Performance Comparison\n'
                          '(Recall = % of real churners caught)',
                          fontweight='bold')
    axes[0, 0].set_ylim(0, 1.1)
    axes[0, 0].tick_params(axis='x', rotation=30)
    axes[0, 0].legend(loc='lower right', fontsize=8)
    axes[0, 0].axhline(0.8, color='grey', linestyle=':', alpha=0.5)
    ...
    for name, r in results.items():
        ...
        axes[0, 1].plot(fpr, tpr, linewidth=lw,
    ...
    axes[1, 0].set_title(
        ...
        f'Caught {tp} churners  |  Missed {fn}  |  False alarms {fp}',
        fontweight='bold')
    ...
    if hasattr(best_model, 'feature_importances_'):
        ...
        axes[1, 1].set_title(
            f'Top 15 Churn Drivers — {best_name}\n'
            '(Business: these are the metrics to monitor and act on)',
            fontweight='bold')
...
def save_artifacts(model, scaler, feature_names: list) -> None:
    ...
    for obj, fname in [(model, 'model.pkl'), (scaler, 'scaler.pkl'),
                       (feature_names, 'feature_names.pkl')]:
        with open(MODELS / fname, 'wb') as f:
            pickle.dump(obj, f)
        print(f"  Saved → models/{fname}")
    print()
    print("  These three files are all you need to make predictions on any new customer.")
